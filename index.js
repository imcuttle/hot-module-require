...
function makeHotRequireFunction(dirname = '', presetOpts = {}) {
  ...
  const resolve = presetOpts.resolvePath
    ? (modulePath) => {
      return presetOpts.resolvePath(modulePath, { dirname, ...presetOpts });
    }
    : function (modulePath) {
  ...
  function removeDependencies(modulePath, deps) {
    ...
    if (!dependent.get(resolvedModulePath) || !dependent.get(resolvedModulePath).length) {
